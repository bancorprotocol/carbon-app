...
function toGmtExplorerSearch(input: ExplorerSearchInput) {
  const { slug, strategies, filter, sort } = input;
  return {
    explore_search_token_pair: slug ? slug.replace('_', '/') : null,
    explore_search_base_token: slug ? slug.split('_').shift()! : null,
    explore_search_quote_token: slug ? slug.split('_').pop()! : null,
...
export const explorerEvents: CarbonEvents['explorer'] = {
  ...
  resultsFilter: ({ slug, strategies, filter, sort }) => {
    sendGTMEvent('explorer', 'exploreSearchResultsFilter', {
      ...toGmtExplorerSearch({ slug, strategies, filter, sort }),
    });
  },
  resultsSort: ({ slug, strategies, filter, sort }) => {
    sendGTMEvent('explorer', 'exploreSearchResultsSort', {
      ...toGmtExplorerSearch({ slug, strategies, filter, sort }),
    });
  },
  manageClick: ({ slug, strategies, strategyEvent, filter, sort }) => {
  ...
  viewOwnersStrategiesClick: (input) => {
    const { slug, strategies, strategyEvent, filter, sort } = input;
