...
export const TradeTriangle = () => {
  ...
  const buy = useGradientOrder('triangle', baseBuy, (next) =>
    saveOrder(next, 'buy'),
  );
  const sell = useGradientOrder('triangle', baseSell, (next) =>
    saveOrder(next, 'sell'),
  );
      ...
      >
        <StrategyChartHistory buy={buy.order} sell={sell.order}>
          <D3EditLine
            ...
            onChange={onSellDrawingChange}
          />
          <D3EditLine
            color="buy"
            drawing={buy.drawing}
            onChange={onBuyDrawingChange}
          />
          <TradeChartContent />
          <D3ChartToday />
