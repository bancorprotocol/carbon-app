    ...
    >
      {items.map((link) => (
        <Link
          ...
          data-testid={link.id}
          className="flex items-center gap-8 border-b border-transparent py-16 px-24 text-white/60 aria-page:tab-focus aria-page:text-white rounded-md xl:max-2xl:rounded-full tab-anchor"
        >
          {link.svg}
          {link.label}
          <Tooltip
